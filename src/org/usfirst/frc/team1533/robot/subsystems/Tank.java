...
public class Tank {
	...
	public void move(){
		...
		double cX = -.99*targetX;
		double cY = -.99*targetY;

		if(joy.getRawButton(ConstantFactory.LEFT_TRIGGER)){
			if(targetX != 0)speedX = ((2*targetX)/(1 + Math.pow(10, (-k * speedX)))) + cX;
			else if(targetX == 0) speedX = 0;
			
			if(targetY != 0) speedY = ((2*targetY)/(1 + Math.pow(10, (-k * speedY)))) + cY;
			else if(targetY == 0) speedY = 0;
		
		}else if(joy.getRawButton(ConstantFactory.RIGHT_TRIGGER)){
			if(targetX != 0) speedX = ((2*targetX)/(1 + Math.pow(10, (-1 * speedX)))) + cX;
			else if(targetX == 0) speedX = 0;
